...
Teoretická informatika poskytuje způsob, jak zkoumat, porozumět a optimalizovat možnosti a limity výpočetních systémů. Hlavním bodem tohoto zkoumání je teorie složitosti, která klasifikuje problémy na základě zdrojů potřebných pro jejich vyřešení, jako je čas a paměť. Jednou z nejvýznamnějších složitostních tříd je třída \( P \), zahrnující problémy, řešitelné v polynomiálním čase na deterministickém Turingov stroji \cite{uti_turing} – tedy takové, jejichž časovou složitost lze vyjádřit jako \( O(n^k) \) \cite{sawa_bigo} pro nějakou konstantu \( k \), kde \( n \) je velikost vstupu – na deterministickém Turingově stroji \cite{uti_turing}. Přestože jsou problémy v této třídě obvykle pokládány za „efektivně řešitelné“, projevují se mezi nimi významné odlišnosti, zejména pokud začneme řešit jejich paralelizovatelnost.
...
\begin{itemize}
    ...
    \item \textbf{Neprázdnost bezkontextových gramatik:} Problém rozhodující, zda daná gramatika generuje neprázdný jazyk \cite{sawa}.
\end{itemize}
