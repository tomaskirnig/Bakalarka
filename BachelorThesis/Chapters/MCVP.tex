...
\section{Reprezentace a parsování výrazů}
...
\begin{enumerate}
    ...
    \item \textbf{Parsování (syntaktická analýza):} Třída \texttt{Parser} implementuje rekurzivní sestupný parser. Metody odpovídají pravidlům gramatiky: \texttt{parseOrExpr()} zpracovává OR výrazy, \texttt{parseAndExpr()} zpracovává AND výrazy a \texttt{parseFactor()} zpracovává závorky nebo proměnné. Parser vytváří uzly typu \texttt{Node}, které tvoří strom reprezentující strukturu obvodu.
\end{enumerate}
...
Vyhodnocení monotónního obvodu probíhá rekurzivním průchodem stromu implementovaným v modulu \texttt{EvaluateTree.js}.

\subsection{Algoritmus vyhodnocení}
\label{sec:MCVPAlgorithm}

Funkce \texttt{evaluateTree()} používá algoritmus do hloubky (DFS) s memoizací:

\begin{enumerate}
    \item \textbf{Listy (proměnné):} Pro uzly typu \texttt{variable} funkce vrací přímo přiřazenou hodnotu (0 nebo 1).
    
    \item \textbf{Vnitřní uzly (operace):} Pro operační uzly funkce rekurzivně vyhodnotí všechny potomky. Pokud je uzel typu AND, výsledek je 1 právě tehdy, když všichni potomci mají hodnotu 1. Pokud je uzel typu OR, výsledek je 1 právě tehdy, když alespoň jeden potomek má hodnotu 1.
...
Pro vzdělávací účely aplikace nabízí krokové vyhodnocení pomocí funkce \texttt{evaluateTreeWithSteps()}. Tato funkce provádí stejný výpočet jako \texttt{evaluateTree()}, ale navíc zaznamenává každý vyhodnocený uzel spolu s hodnotami jeho potomků do pole kroků. Uživatel tak může sledovat, jak se hodnoty šíří od listů k~kořeni stromu, a lépe pochopit princip vyhodnocování.
...
\begin{itemize}
    \item \textbf{Přidávat uzly:} Kliknutím do prázdné oblasti grafu lze vytvořit nový uzel. Uživatel vybere typ uzlu (proměnná s hodnotou 0, proměnná s hodnotou 1, hradlo AND nebo hradlo OR).
    
    \item \textbf{Vytvářet hrany:} Táhnutím myši z jednoho uzlu do druhého vytvoří hranu reprezentující tok dat v obvodu.
    
    \item \textbf{Odstraňovat prvky:} Pravým kliknutím na uzel nebo hranu lze prvek odstranit z obvodu.
...
Aplikace obsahuje generátor náhodných monotónních obvodů v modulu \texttt{Generator.js}. Uživatel může specifikovat:

\begin{itemize}
    \item \textbf{Počet proměnných:} Určuje, kolik vstupních uzlů bude obvod obsahovat.
    \item \textbf{Počet operací:} Určuje, kolik vnitřních uzlů (hradel) bude obvod obsahovat.
    \item \textbf{Strukturu:} Algoritmus zajišťuje, že výsledný obvod je platný orientovaný acyklický graf s jedním kořenovým uzlem.
\end{itemize}
