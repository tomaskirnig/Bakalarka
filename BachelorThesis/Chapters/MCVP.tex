...
Aplikace obsahuje generátor náhodných monotónních obvodů v modulu \texttt{Generator.js}. Uživatel zadává dva parametry:

\begin{itemize}
    \item \textbf{Počet hradel:} Určuje, kolik vnitřních uzlů (hradel AND a OR) bude obvod obsahovat.
    \item \textbf{Počet proměnných:} Určuje, kolik vstupních uzlů (listů) bude obvod obsahovat.
\end{itemize}

Generátor postupně vytváří náhodný obvod: nejprve vygeneruje zadaný počet proměnných s náhodnými hodnotami (0 nebo 1), poté přidává operační uzly. Každý nový operační uzel náhodně vybere dva existující uzly jako potomky a náhodně zvolí operaci (AND nebo OR). Algoritmus zajišťuje, že výsledný obvod je platný orientovaný acyklický graf.
