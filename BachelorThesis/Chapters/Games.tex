...
\section{Formát vstupu a reprezentace}
\label{sec:GamesInput}

Aplikace nabízí několik způsobů zadání herního grafu. Uživatel může zadat graf textově, vytvořit ho interaktivně nebo načíst z připravených sad.

\subsection{Textový formát}
\label{sec:GamesTextFormat}

Textové zadání využívá jednoduchý formát:
...
\begin{itemize}
    \item \texttt{nodes:} definuje uzly ve formátu \texttt{id:player}, kde \texttt{id} je identifikátor uzlu a \texttt{player} určuje, který hráč je v této pozici na tahu (1 nebo 2).
    \item \texttt{edges:} definuje hrany ve formátu \texttt{source->target}, reprezentující možný tah z pozice \texttt{source} do pozice \texttt{target}.
\end{itemize}

Kromě samotného grafu musí uživatel také určit počáteční pozici.

\subsection{Parsování vstupu}
\label{sec:GamesParsing}

Parsování textového vstupu implementuje modul \texttt{Parser.js}. Funkce \texttt{parseExpressionToTree()} zpracovává vstupní řetězec v těchto krocích:

\begin{enumerate}
    \item Rozdělí výraz na část s uzly a část s hranami pomocí oddělovače středníkem.
    \item Zpracuje definice uzlů – pro každý uzel vytvoří objekt \texttt{GamePosition} s ID a přiřazeným hráčem.
    \item Zpracuje definice hran – pro každou hranu propojí odpovídající uzly a aktualizuje seznamy potomků a rodičů.
    \item Vrátí strukturu \texttt{GameGraph} obsahující všechny pozice a odkaz na počáteční pozici.
\end{enumerate}

Výsledná struktura používá třídy \texttt{GamePosition} a \texttt{GameGraph} definované v modulu \texttt{NodeClasses.js}.
...
\begin{itemize}
    ...
    \item \textbf{Vytvářet hrany:} Po označení prvního uzlu aktivujeme režim přidávání hrany. Následným kliknutím na druhý uzel vytvoříme hranu mezi nimi.
...
Aplikace průběžně validuje graf – upozorní uživatele, pokud není nastaven počáteční uzel nebo pokud graf obsahuje cykly (což by porušilo požadavek na DAG).
