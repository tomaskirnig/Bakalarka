...
\begin{itemize}
    ...
    \item \textbf{Vizualizační komponenty:} Starají se o vykreslení grafů a stromů pomocí knihovny \emph{react-force-graph-2d}.
\end{itemize}
...
Pro modelování logického obvodu byl využit orientovaný acyklický graf (DAG). Každý uzel obvodu je reprezentován instancí třídy \texttt{Node}. Tato třída, jak ukazuje třídní diagram na obrázku \ref{fig:mcvp-class}, uchovává informaci o typu operace (AND, OR) nebo hodnotě proměnné. Seznam odkazů na potomky umožňuje průchod grafem při vyhodnocování.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.3\textwidth]{IMGs/MCVP_class.png}
...
Třída \texttt{Grammar} (obrázek \ref{fig:grammar-class}) spravuje množiny symbolů a seznamy pravidel. Každé pravidlo propojuje levou stranu (neterminál) s~polem symbolů na pravé straně. Tato struktura umožňuje snadno identifikovat produktivní neterminály.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.25\textwidth]{IMGs/Grammar_class.png}
...
\begin{itemize}
    \item \textbf{Ovládací panel:} Horní část obrazovky, kde uživatel volí způsob zadání vstupu a spouští simulace (viz obrázek \ref{fig:ui-navigation}).
    \item \textbf{Interaktivní plátno:} Hlavní plocha pro vizualizaci grafů, kde uživatel může s~prvky přímo manipulovat (viz obrázek \ref{fig:ui-canvas}).
    \item \textbf{Informační panely:} Modální okna nebo postranní panely zobrazující detaily o probíhajícím výpočtu nebo krokovém převodu (viz obrázek \ref{fig:ui-info-panel}).
\end{itemize}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{IMGs/navigation.png}
    \caption{Ovládací panel s~navigací a výběrem vstupu}
    \label{fig:ui-navigation}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{IMGs/InteractiveCanvas.png}
    \caption{Interaktivní plátno pro vizualizaci a editaci grafů}
    \label{fig:ui-canvas}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{IMGs/informationPanel.png}
    \caption{Informační panel zobrazující výsledek analýzy}
    \label{fig:ui-info-panel}
\end{figure}

Pro vizualizaci byla použita knihovna \emph{react-force-graph-2d} \cite{reactforcegraph2024}, která k~výpočtu fyzikální simulace pro automatické rozmísťování uzlů a hran využívá engine \emph{D3.js} \cite{d3js2024}. Stylování je realizováno frameworkem \emph{Bootstrap 5} \cite{bootstrap2024}, který zajišťuje jednotný vzhled.