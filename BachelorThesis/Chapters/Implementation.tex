...
\begin{enumerate}
    \item \textbf{Inicializace:} Algoritmus vytvoří prazdnou množinu produktivních neterminálů a frontu pro zpracování. Všechna pravidla jsou zpočátku neoznačená.
    
    \item \textbf{Nalezení základních produktivních pravidel:} Projdou se všechna pravidla gramatiky a identifikují se ta, jejichž pravá strana obsahuje pouze terminály nebo je prázdná ($\varepsilon$). Neterminály na levé straně těchto pravidel jsou okamžitě označeny jako produktivní a vloženy do fronty.
...
Časová složitost je \( O(|P| \cdot l) \), kde \( |P| \) je počet pravidel gramatiky a \( l \) je průměrná délka pravé strany pravidla. V nejhorším případě může být každé pravidlo kontrolováno tolikrát, kolik obsahuje symbolů.
