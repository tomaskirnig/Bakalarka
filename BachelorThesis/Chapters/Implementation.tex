...
\begin{enumerate}
    ...
    \item \textbf{Parsování (syntaktická analýza):} Třída \texttt{Parser} implementuje rekurzivní sestupný parser. Parser je strukturován podle priority operátorů: \texttt{parseOrExpr()} zpracovává OR výrazy, \texttt{parseAndExpr()} zpracovává AND výrazy a \texttt{parseFactor()} zpracovává závorky a proměnné. Parser vytváří uzly typu \texttt{Node} (viz obrázek \ref{fig:mcvp-class-impl}), které tvoří strukturu grafu reprezentující zadaný obvod.
\end{enumerate}

Návrh třídy \texttt{Node} a celková architektura datových struktur je detailně popsána v sekci \ref{sec:MCVPDesignStructure}. Výsledkem parsování je orientovaný graf (obrázek \ref{fig:mcvp-class-impl}), kde vstupní uzly jsou proměnné s přiřazenými hodnotami a vnitřní uzly reprezentují logické operace.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{IMGs/MCVP_class.png}
    \caption{Třídní diagram pro hlavní třídy MCVP}
    \label{fig:mcvp-class-impl}
\end{figure}
