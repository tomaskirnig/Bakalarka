\chapter{Problém neprázdnosti jazyka \\ bezkontextové gramatiky}
\label{sec:Grammars}
...
Podobně jako u předchozích problémů, i zde P-úplnost vyplývá ze sekvenčních závislostí: produktivita neterminálu závisí na produktivitě jiných neterminálů, což brání paralelnímu zpracování.
...
\begin{enumerate}
    ...
    \item \textbf{Sestavení gramatiky:} Vytvoření instance třídy \texttt{Grammar} (viz obrázek \ref{fig:grammar-class}) s vygenerovanými symboly a pravidly.
\end{enumerate}
...
Modul \texttt{GrammarEvaluator.js} obsahuje implementaci algoritmu pro vyhodnocení problému neprázdnosti jazyka bezkontextové gramatiky. Algoritmus zjišťuje, zda je počáteční symbol \( S \) produktivní – tedy zda z něj lze odvodit slovo složené pouze z terminálů nebo prázdný řetězec \( \epsilon \).
...
\begin{enumerate}
    \item \textbf{Inicializace:} Vytvoří se prázdná množina produktivních symbolů \( \mathcal{P} \).
    
    \item \textbf{Základní krok:} Do \( \mathcal{P} \) se přidají všechny neterminály \( A \), které mají pravidlo \( A \to w \), kde \( w \in \Sigma^* \) (pravá strana obsahuje pouze terminály nebo je prázdná).
    
    \item \textbf{Iterativní rozšiřování:} Opakovaně se procházejí všechna pravidla. Pokud existuje pravidlo \( A \to X_1 X_2 \dots X_k \), kde všechny symboly \( X_i \) jsou buď terminály nebo již jsou v množině \( \mathcal{P} \), přidá se \( A \) do \( \mathcal{P} \).
    
    \item \textbf{Ukončení:} Krok 3 se opakuje, dokud se množina \( \mathcal{P} \) mění.
...
\begin{enumerate}
    \item \textbf{Výběr pravidla:} Pro každý neterminál se náhodně vybere jedno z jeho produktivních pravidel. Při větší hloubce derivace (> 20 kroků) jsou preferována nerekurzivní pravidla, aby se předešlo nekonečným derivacím.
    ...
    \begin{itemize}
        \item Pro každý neterminál se vytvoří uzel s unikátním identifikátorem
        \item Náhodně se vybere jedno z jeho produktivních pravidel
        \item Pro každý symbol na pravé straně pravidla rekurzivně vzniknou potomci (zvýší se hloubka o 1)
...
\begin{itemize}
    \item \textbf{Seznam pravidel gramatiky:} Zobrazuje všechna produktivní pravidla. Aktuálně kontrolované pravidlo je vizuálně zvýrazněno žlutým pozadím a oranžovým ohraničením. Pravidla, jejichž levá strana je již produktivní, jsou označena zeleným štítkem s textem \uv{Produktivní}.
    ...
    \item \textbf{Vysvětlení kroku:} Textový popis aktuální operace algoritmu, například \uv{Pravidlo \( A \to a \) obsahuje pouze terminály. \( A \) je produktivní} nebo \uv{Pravidlo se stalo produktivním díky \( B \). \( A \) přidán mezi produktivní}.
\end{itemize}
...
Aktuální pozice je indikována čítačem ve formátu \uv{Krok \( x \) z \( n \)}, kde \( n \) představuje celkový počet kroků. Tato interaktivní vizualizace umožňuje sledovat, jak algoritmus postupně identifikuje produktivní neterminály na základě již známých produktivních symbolů a jak závislosti mezi pravidly určují pořadí jejich vyhodnocení.