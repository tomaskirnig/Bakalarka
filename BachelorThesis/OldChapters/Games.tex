...
\subsection{Vnitřní reprezentace herního grafu}
\label{sec:GamesInternalRepresentation}

Pro reprezentaci struktury herního grafu v paměti aplikace slouží dvojice objektových tříd \texttt{GamePosition} a \texttt{GameGraph}, jejichž vztah ilustruje třídní diagram na obrázku \ref{fig:cg-class}. 

Třída \texttt{GamePosition} modeluje jednotlivou herní pozici jako uzel grafu. Každá instance uchovává identifikátor hráče, který je v dané pozici na tahu, spolu se seznamy předchůdců a následovníků. Tyto seznamy umožňují efektivní navigaci grafem při retrográdní analýze i při zobrazování možných tahů.

Třída \texttt{GameGraph} představuje kontejner sdružující všechny herní pozice do jednoho celku. Tato třída spravuje kolekci všech instancí \texttt{GamePosition} a uchovává referenci na počáteční uzel, ze kterého hra začíná. Struktura umožňuje rychlý přístup k~libovolné pozici v~grafu a podporuje operace potřebné pro analýzu výherní strategie.
...
\begin{itemize}
    ...
    \item \textbf{Vysvětlení:} U každého kroku je zobrazeno vysvětlení, proč došlo ke změně stavu (např. \uv{Z pozice X lze táhnout do prohrávající pozice Y}).
\end{itemize}
