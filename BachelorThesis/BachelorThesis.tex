...
\EnglishThesisTitle{Component of Teaching Server for Theoretical Computer Science - P-complete problems
}
...
\CzechAbstract{Tato bakalářská práce se zabývá vývojem výukové webové aplikace pro demonstraci P-úplných problémů. Cílem je usnadnit studentům pochopení a procvičování těchto problémů. Aplikace se zaměřuje na tři P-úplné problémy: \emph{Monotone Circuit Value Problem} (MCVP), \emph{problém neprázdnosti jazyka bezkontextové gramatiky} a \emph{kombinatorické hry na grafu}. Práce zahrnuje implementaci modulů pro interaktivní zadávání vstupů (ručně, náhodnou generací nebo výběrem z~připravené sady) a simulaci jejich řešení. Uživatel může také sledovat krokovatelný převod instance MCVP na další zmíněné problémy a následně jejich řešení.}

\CzechKeywords{P-úplnost, Monotone Circuit Value Problem, logaritmické redukce, kombinatorické hry na grafu, problém neprázdnosti jazyka bezkontextové gramatiky, polynomiální redukce, vizualizace algoritmů, interaktivní výuková webová aplikace}

\EnglishAbstract{This bachelor's thesis focuses on the development of a teaching-oriented web application for illustrating P-complete problems. The main goal is to facilitate students' understanding and practice of such tasks. The application focuses on the \emph{Monotone Circuit Value Problem} (MCVP), \emph{Context-Free Grammar Non-emptiness Problem}, and \emph{Combinatorial Games on Graphs}. The project implements modules for interactive input of problem instances (manually, via random generation, or by selecting from a pre-defined set) and provides a simulation of their solutions. Users can also observe a step-by-step reduction from an MCVP instance to the other mentioned problems and subsequently explore how those are solved. The resulting application demonstrates key concepts of theoretical computer science, including the notion of P-completeness, and provides a flexible basis for educational use.}

\EnglishKeywords{P-completeness, Monotone Circuit Value Problem, logspace reductions, combinatorial games on graphs, context-free grammar non-emptiness problem, polynomial-time reductions, algorithm visualization, interactive educational web application}

\AddAcronym{MCVP}{Monotone Circuit Value Problem}
\AddAcronym{AND}{Logický součin -- hradlo typu AND}
\AddAcronym{OR}{Logický součet -- hradlo typu OR}
...
\input{Chapters/AnalysisAndDesign.tex}
